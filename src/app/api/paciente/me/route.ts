...
export async function GET(req: NextRequest) {
  ...
  const [account, bookings, requests] = await Promise.all([
    prisma.patientAccount.findUnique({ where: { email }, select: { id: true, email: true, mustChangePassword: true, rut: true, firstName: true, secondName: true, firstSurname: true, secondSurname: true, birthDate: true, gender: true, occupation: true, companion: true, address: true, region: true, commune: true, phone: true, educationLevel: true, emergencyContact: true, clinicalUpdates: { select: { id: true, status: true, createdAt: true }, orderBy: { createdAt: 'desc' }, take: 3 } } }),
